...
function drawInlineSVG(ctx, svgElement, callback) {
...
finalDiv.append("button")
    ...
    .on("click", function () {
        // This function triggers a full map draw, good trace point
        doMap(finalSVG, defaultParams);
    });

finalDiv.append("button")
    .text("populate canvas")
    .on("click", function() {
        console.log('working');
        drawInlineSVG(document.getElementById("finalCanvas").getContext('2d'), document.getElementById("finalMap"), function() {
            console.log(document.getElementById("finalCanvas").toDataURL());
        });
    });
